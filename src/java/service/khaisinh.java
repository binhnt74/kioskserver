 ...
 * @author ngocduc
 */
@WebService(serviceName = "khaisinh")
...
//                }
        ...
        }else returnWsobject.notification = "Dịch vụ chưa sẵn sàng hoặc bạn không thể thực hiện dịch vụ này.\n"
                + "Vui lòng thử lại sau.";// dịch vụ chưa có trong kiosk hoặc quyền của user.
        //het test
        return returnWsobject;
    }
        ...
        if(connect.getProtocol(wsobject.frame.idService).equals("Webserver")){
            ...
            try {
                ...
                if((wsobject.frame.nameFrame.equals("loginWeb") || wsobject.frame.nameFrame.contains("formDangKyTaiKhoanDVCQG")
                    ...
                    oo.frame.setData(frame.getData());
                    for (ComponentClient comp : client.getFrame().lstComponent) {
                        if(comp.getType().equals("Label") && comp.getLinkWeb()  != null){
                    ...
                    oo.notificationKQ = notification;
                    return oo;
                }
                // Sau submit lam gi?
                ...
                wsobject.notification = "Đã có lỗi xảy ra, xin vui lòng thử lại sau.";
                return wsobject;
            }
        }else{
        ...
        if (wsobject.frame.nameFrame.equals("loginWeb")) {
            ...
            while (true) {                
                if (client.getDriver().driver.getCurrentUrl().equals(expectedUrl)) {
                    ...
                    if(!client.getDriver().driver.getCurrentUrl().equals("https://dvc.bacninh.gov.vn/dangnhap"))
                        client.getDriver().driver.get("https://dvc.bacninh.gov.vn/dangnhap");
